...
def _to_float(value) -> float:
    """Convert Series/array/scalar to float, handle edge cases"""
    try:
...
def _call_gemini_for_recommendation(ticker: str, df: pd.DataFrame) -> tuple[str, int, dict]:
    """Ask Gemini to analyze the stock data and give us a recommendation"""
    ...
    def safe(v, default=None):
        """Helper to extract single value from potentially nested data"""
        try:
    ...
    close_series = pick_series(recent, ["Close", "close"])
...
def main():
    ...
    st.caption("News Sentiment based stock position recommendation using LLMs")
